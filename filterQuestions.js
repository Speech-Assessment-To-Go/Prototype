...
export function filterQuestions(number, structure, complexity, pastQuestions = []) {
	...
	* Parameters:
		...
		* pastQuestions, an array of integers initially representing what questions have already been loaded in the assessment
	...
	*/
	...
	let i = 0; //represent where we are in allQuestions
	
	while (pastQuestions.length < allQuestions.length && addedQuestions.length < number) { //while we haven't exhausted our list of questions and we haven't reached the requested number
		...
		pastQuestions.push(i); //note that we've looked at this question so we don't look at it again
